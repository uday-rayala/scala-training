...
class TimeSpec extends Specification { def is =
  ...
  "Calling Time.isIncreasing" ^
    ...
    `should return true for a Seq with 0 or 1 elements` ^
    `should return true for a Seq with increasing elements` ^
    `should return false for a Seq with not-increasing elements` ^
  end
