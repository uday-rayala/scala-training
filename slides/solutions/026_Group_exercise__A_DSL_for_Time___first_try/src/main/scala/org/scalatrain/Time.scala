...
object Time extends XmlSerializable[Time] {
  ...
  implicit def intToTimeBuilder(hours: Int): TimeBuilder = {
...
private[scalatrain] class TimeBuilder(val hours: Int, val minutes: Int = 0) {

  def h = new TimeBuilder(hours)

  def apply(minutes: Int) = new TimeBuilder(hours, minutes)

  def m = Time(hours, minutes)
}
