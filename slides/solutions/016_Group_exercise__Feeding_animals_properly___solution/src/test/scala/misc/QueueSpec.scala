...
class QueueSpec extends Specification { def is =
  ...
  p^
  "Calling Queue.enqueue" ^
    `should return a new Queue with the enqueued element at the end` ^
  p^
  ...
  def `should return a new Queue with the enqueued element at the end` =
    Queue(1, 2) enqueue 3 must_== Queue(1, 2, 3)
